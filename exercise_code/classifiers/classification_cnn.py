...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        # conv kernel
        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride=stride_conv, padding=(kernel_size-1)//2)
        self.conv.weight.data.mul_(weight_scale)

        # fully connected (linear) layers
        self.fc1 = nn.Linear((num_filters)*((height-pool)//stride_pool+1)*((width-pool)//stride_pool+1), hidden_dim)
    ...
    def forward(self, x):
        ...
        # Max pooling on relu of conv layer
